...
rust-objcopy --binary-architecture=riscv64 --strip-all -O binary target/riscv64gc-unknown-none-elf/release/putchar ./putchar.bin

rust-objcopy --binary-architecture=riscv64 --strip-all -O binary target/riscv64gc-unknown-none-elf/release/putchar_d ./putchar_d.bin
...
app_num=2
printf '%02x' $app_num | xxd -r -p | dd of=apps.bin conv=notrunc bs=1 seek=0
app1_size=$(stat -c%s ./putchar_d.bin)
printf '%04x' $app1_size | xxd -r -p | dd of=apps.bin conv=notrunc bs=1 seek=1
dd if=./putchar_d.bin of=./apps.bin conv=notrunc bs=1 seek=3
app2_size=$(stat -c%s ./putchar.bin)
printf '%04x' $app2_size | xxd -r -p | dd of=apps.bin conv=notrunc bs=1 seek=`expr 3 + $app1_size`
dd if=./putchar.bin of=./apps.bin conv=notrunc bs=1 seek=`expr 5 + $app1_size`
